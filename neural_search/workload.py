...
class NeuralQueryParamSource(QueryParamSource):
    ...
    def params(self):
        params = self._params
        use_semantic_field = params.get('use_semantic_field', 'False')
        if use_semantic_field == 'True':
            path_to_embedding = "body.query.neural.passage_embedding"
            embedding_query = get_by_path(params, path_to_embedding)
        else:
...
def register(registry):
    ...
    registry.register_param_source("create-index-with-semantic-field", CreateIndexWithSemanticFieldParamSource)
